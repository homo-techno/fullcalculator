...
const newImports = fs.readFileSync(path.join(__dirname, 'new-imports-batch7.txt'), 'utf8').trim();
const newRegs = fs.readFileSync(path.join(__dirname, 'new-regs-batch7.txt'), 'utf8').trim();
...
if (lastImportIdx === -1 || closingIdx === -1) { console.error('Could not find insertion points'); process.exit(1); }
