import type { CalculatorDefinition } from "./types";
import { formatNumber } from "@/lib/utils";

export const russiaIncomeTaxCalculator: CalculatorDefinition = {
  slug: "russia-income-tax-calculator",
  title: "Russia Income Tax Calculator",
  description: "Free Russia income tax calculator. Calculate your tax liability with current progressive brackets and deductions.",
  category: "Finance",
  categorySlug: "finance",
  icon: "$",
  keywords: ["russia income tax calculator", "russia tax calculator", "russia tax calculator"],
  variants: [{
    id: "standard",
    name: "Russia Income Tax",
    description: "Free Russia income tax calculator",
    fields: [
      { name: "income", label: "Annual Taxable Income", type: "number", prefix: "₽", min: 0 },
    ],
    calculate: (inputs) => {
      const income = inputs.income as number;
      if (!income || income <= 0) return null;
      const brackets = [{l:2400000,r:0.13},{l:5000000,r:0.15},{l:20000000,r:0.18},{l:50000000,r:0.2},{l:Infinity,r:0.22}];
      let tax = 0, rem = income, prev = 0, marginal = 0;
      for (const s of brackets) { const t = Math.min(rem, s.l - prev); if (t <= 0) break; tax += t * s.r; marginal = s.r; rem -= t; prev = s.l; }
      return {
        primary: { label: "Income Tax", value: "₽" + formatNumber(tax) },
        details: [
          { label: "Taxable income", value: "₽" + formatNumber(income) },
          { label: "Effective rate", value: formatNumber((tax / income) * 100) + "%" },
          { label: "Marginal rate", value: (marginal * 100).toFixed(0) + "%" },
          { label: "After-tax income", value: "₽" + formatNumber(income - tax) },
          { label: "Monthly after-tax", value: "₽" + formatNumber((income - tax) / 12) },
        ],
        note: "2025 progressive NDFL brackets. Previously flat 13%/15%.",
      };
    },
  }],
  relatedSlugs: ["tax-calculator", "salary-calculator"],
  faq: [
    { question: "How is income tax calculated in Russia?", answer: "Russia uses a progressive tax system where higher income portions are taxed at higher rates." },
    { question: "What is the top tax rate in Russia?", answer: "The highest marginal rate is 22%." },
  ],
  formula: "Tax = Sum of (income in bracket × rate)",
};
